...
| `competition` | string | M | — | Competition identifier — machine-readable, unique within the tournament; the same value as `competition` in `software/record` and the competition `id` in the tournament identity message (Section 31.5) |
...
| `competition` | string | M | — | Identifier of the competition this slot belongs to — same value as `competition` in `software/match`. When the venue is bridged to a cloud broker, this is the competition `id` from the tournament identity message (Section 31.5); see Section 31.4 |
...
|-------|----------------------------|
| Slot assigned to piste (scoresheet `NEXT_SLOT`, see below) | Full payload; all `result` fields null; `active_bout` absent — no bout is active until the apparatus itself requests one |
| Bout confirmed (ACK) | `bouts[n].result` filled in; `active_bout` reverts to absent — the CMS does not predict what bout comes next; see the `active_bout` field note above |
| Apparatus `NEXT`/`PREV` (Section 24) | `active_bout` set to the bout the apparatus actually requested |
...
| Piste transfer | Full payload on the new piste's topic — same `slot_id`, same bouts, same results, same `active_bout`; `bouts[n].annotations` populated for completed bouts |
...
A piste transfer is not assumed to preserve any physical continuity — the new piste may have a different apparatus, a different location, or both. Anything the CMS itself holds (the bout list, results so far, the officiating roster, and — carried over as a known fact, not a new prediction — `active_bout`) is republished as above. Anything only the old apparatus knew (current score, cards, priority, elapsed clock time, UW2F timer) cannot be recovered from `software/record` alone; the CMS separately seeds the new piste's apparatus with `software/score`, `software/clock`, and `software/uw2f` set to the old apparatus's last-known values, so an in-progress bout resumes rather than restarting from zero. See Sections 11, 13, and 19.
...
Published when a control event occurs. This topic is bidirectional — it carries commands from apparatus to software, from software to apparatus, from remote controls to apparatus, and from the scoresheet or the video referee system to software. The publisher segment in the topic identifies the source: `apparatus/control`, `software/control`, `remote/control`, `scoresheet/control`, or `var/control`. A receiver that encounters an unknown command value SHOULD ignore it.
...
```
openpiste/{country}/{year}/{month}/{day}/{tournament_id}/{piste_id}/{publisher}/{message_type}
```
...
| `{tournament_id}` | Machine-readable tournament identifier — mandatory | `bel-nat-champ-2026` |
...
```
openpiste/BEL/2026/06/15/bel-nat-champ-2026/17/apparatus/lights
```

on the cloud broker.

A tournament is a collection of competitions held at one venue, and one venue hosts one tournament at a time. The whole prefix is therefore fixed for the lifetime of the tournament: it does not depend on which competition a piste is currently running (Section 31.4). This is what allows the bridge to be a static, native broker bridge configuration (Section 31.6).

### 31.3 Rationale for the topic structure

**Global uniqueness without a global registry.** No single segment needs to be globally unique on its own. The combination of country, date, and tournament identifier creates global uniqueness through hierarchy — the same way a postal address works. `bel-nat-champ-2026` only needs to be unique within `BEL/2026/06/15/`, which is a very small namespace. A national federation can maintain its own list of tournament identifiers without coordinating with any global authority. Before configuring a bridge, an operator SHOULD check that no identity message (Section 31.5) from a different tournament is already retained at the intended path.
...
openpiste/+/+/+/+/+/+/apparatus/score     # every apparatus score, at every venue in the world
openpiste/+/+/+/+/+/identity              # every tournament currently on the cloud broker
```

Note: MQTT wildcards use `+` (single segment) and `#` (all remaining segments). The `*` character is not a valid MQTT wildcard.

**Competition is not a topic segment.** A subscriber interested in a single competition subscribes to its tournament and filters on the `competition` field carried by the piste's messages (Section 31.4). See Section 31.4 for why the competition cannot be part of the path.
...
A tournament runs several weapon and category events, often simultaneously, and pistes are routinely shared between them: a block of pistes hosts one competition's pools in the morning and another's in the afternoon, and a single final piste hosts the finals of every event. Which competition a piste belongs to is therefore not a property of the piste — it changes over the day, sometimes from one slot to the next.

For this reason the competition is carried as **data**, not as a topic segment. Every slot assigned to a piste belongs to exactly one competition, and the CMS identifies it in the `competition` field of:

- `software/record` (Section 17) — retained, so a subscriber joining at any time learns immediately which competition each piste is currently running;
- `software/match` (Section 16) — per bout, not retained.

The value of `competition` is the competition identifier listed in the tournament's identity message (Section 31.5), which maps it to a full description of the competition.

```
openpiste/ITA/2026/06/15/eur-champ-2026/17/software/record   # { ..., "competition": "efj-eq", ... }   (morning)
openpiste/ITA/2026/06/15/eur-champ-2026/17/software/record   # { ..., "competition": "esf-foil", ... } (afternoon)
```

A topic segment cannot follow these changes: a bridge rewriting the competition into the path would have to know, for every message, which slot is currently active on the piste — knowledge only the CMS has — and could no longer be a native broker bridge.

### 31.5 Tournament identity message

The bridge publishes a retained identity message to the cloud broker when the tournament starts, and republishes it whenever the tournament's competition list changes. This serves as the discovery layer — a subscriber can watch `openpiste/+/+/+/+/+/identity` to find all tournaments currently on the cloud broker, or narrow the subscription to a specific country or date range. The identity message SHOULD be cleared (an empty retained payload) when the tournament ends.

**Topic:** `openpiste/{country}/{year}/{month}/{day}/{tournament_id}/identity`
**QoS:** 1
**Retained:** Yes
...
```json
{
  ...
  "competitions": [
    ...
    {
      "id":       "esf-foil",
      "name":     "Senior Women's Foil Individual",
      "weapon":   "F",
      "type":     "I",
      "category": "Senior",
      "gender":   "F"
...
### 31.5.1 Identity message fields
...
**Competition fields** (one entry per competition in `competitions`):

| Field | Type | M/O | Description |
|-------|------|-----|-------------|
| `id` | string | M | Machine-readable competition identifier, unique within the tournament. This is the value of `competition` in `software/record` and `software/match` |
...
Because the cloud prefix depends only on the tournament (Section 31.2), a bridge needs exactly one piece of per-event configuration: the tournament's country, start date, and identifier. It does not need to know which competition any piste is running — that information travels inside the relayed messages themselves (Section 31.4). The bridge configuration is therefore static for the whole tournament and can be expressed natively in the broker's own bridge configuration, with no custom relay software.

**Topics a bridge relays.** A bridge SHOULD relay the piste topics of each publisher role it intends to publish — `openpiste/+/{publisher}/#` for `apparatus`, `software`, `remote`, `var`, `scoresheet`, and any vendor-defined roles (Section 29.5) the operator chooses to include. A bridge **MUST NOT** relay `openpiste/_provision/#` (Section 30.5): provisioning responses carry freshly issued client credentials, and the topic has the same shape as a piste topic, so a wildcard such as `openpiste/+/+/+` would relay them. Relaying by publisher role, as above, avoids this. The bridge relays in one direction only, from the local broker to the cloud broker.

**Mosquitto example.** For the tournament in Section 31.5:

```
connection openpiste-cloud
address mqtt.example.org:8883
remote_clientid <unique per venue>
topic +/apparatus/#  out 1 openpiste/ openpiste/ITA/2026/06/15/eur-champ-2026/
topic +/software/#   out 1 openpiste/ openpiste/ITA/2026/06/15/eur-champ-2026/
topic +/remote/#     out 1 openpiste/ openpiste/ITA/2026/06/15/eur-champ-2026/
topic +/var/#        out 1 openpiste/ openpiste/ITA/2026/06/15/eur-champ-2026/
topic +/scoresheet/# out 1 openpiste/ openpiste/ITA/2026/06/15/eur-champ-2026/
```

`remote_clientid` MUST be unique per venue: many venues bridge to the same cloud broker, and a broker disconnects an existing client when another connects with the same client identifier.

**How the tournament information reaches the bridge** remains an open question, with two options:

**Option A — Manual bridge configuration.** The bridge operator enters the tournament identity once, at setup. Since the configuration is static for the tournament, this is a single step per event.

**Option B — CMS-published identity.** The CMS publishes the tournament identity message (Section 31.5) to a well-known local topic, and a bridge-side component uses it to configure the cloud prefix and republish the identity to the cloud. This keeps the competition list in the identity message in step with the CMS automatically.
...
**Cloud bridge CMS integration.** Whether the bridge obtains the tournament identity by manual configuration or from a CMS-published local message is not yet resolved. See Section 31.6.
